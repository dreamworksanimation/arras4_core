...
rez-env arras4_node -c "arras4_node --use-cgroups --enforce-memory --loan-memory -l 5 --consul-host q_con_jose_configsvc.db.gld.dreamworks.net  --athena-env dev --use-color 0 --exclusive-user --no-local-rez" |& tee /tmp/arras/node_service.log
